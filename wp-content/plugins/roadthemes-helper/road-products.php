...
class RoadProducts {
	...
	public function road_vc_shortcodes() {
		...
		$order_by_values = array(
			'',
			__( 'Date', 'sneaker' )          => 'date',
			__( 'ID', 'sneaker' )            => 'ID',
			__( 'Author', 'sneaker' )        => 'author',
			__( 'Title', 'sneaker' )         => 'title',
			__( 'Modified', 'sneaker' )      => 'modified',
			__( 'Random', 'sneaker' )        => 'rand',
			__( 'Comment count', 'sneaker' ) => 'comment_count',
			__( 'Menu order', 'sneaker' )    => 'menu_order',
		);

		$order_way_values = array(
			'',
			__( 'Descending', 'sneaker' ) => 'DESC',
			__( 'Ascending', 'sneaker' )  => 'ASC',
		); 
		 
	//Sale products Roadthemes
	vc_map( array(
		'name'        => __( 'RT Sale products', 'sneaker' ),
		'base'        => 'sale_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'sneaker' ),
		'description' => __( 'List all products on sale', 'sneaker' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
					'heading'     => esc_html__( 'Per page', 'sneaker' ),
					'value'       => 12,
					'save_always' => true,
					'param_name'  => 'per_page',
					'description' => esc_html__( 'How much items per page to show', 'sneaker' ),
				),
					array(
					'type'        => 'textfield',
					'heading'     => __( 'Columns', 'sneaker' ),
					'value'       => 4,
					'save_always' => true,
					'param_name'  => 'columns',
					'description' => __( 'How much columns grid', 'sneaker' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'sneaker' ),
					'param_name'  => 'orderby',
					'value'       => $order_by_values,
					'save_always' => true,
					'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'sneaker' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Sort order', 'sneaker' ),
					'param_name'  => 'order',
					'value'       => $order_way_values,
					'save_always' => true,
					'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'sneaker' ), '<a href="http://codex.wordpress.org/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">WordPress codex page</a>' ),
				), 
	...
	vc_map( array(
		'name'        => __( 'RT Featured products', 'sneaker' ),
		'base'        => 'featured_products_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'sneaker' ),
		'description' => __( 'Display products set as "featured"', 'sneaker' ),
		'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name'        => __( 'RT Recent products', 'sneaker' ),
	'base'        => 'recent_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sneaker' ),
	'description' => __( 'Lists recent products', 'sneaker' ),
	'params'      => array_merge( 
		array(
			array(
				'type'        => 'textfield',
	...
	vc_map( array(
	'name' => __( 'Top Rated Products', 'sneaker' ),
	'base' => 'top_rated_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category' => __( 'Theme', 'sneaker' ),
	'description' => __( 'List all top rated products', 'sneaker' ),
	'params'      => array_merge( 
		array(
			array(
				'type' => 'textfield',
				'heading' => __( 'Per page', 'sneaker' ),
				'value' => 12,
				'param_name' => 'per_page',
				'save_always' => true,
				'description' => __( 'How much items per page to show', 'sneaker' ),
			),
			array(
				'type' => 'textfield',
				'heading' => __( 'Columns', 'sneaker' ),
	...
	vc_map( array(
	'name'        => __( 'RT Bestselling products', 'sneaker' ),
	'base'        => 'bestselling_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sneaker' ),
	'description' => __( 'List best selling products on sale', 'sneaker' ),
	'params'      => array_merge( 
		array(
				array(
					'type'        => 'textfield',
	...
	//products Roadthemes
	vc_map( array(
	'name'        => __( 'RT Products', 'sneaker' ),
	'base'        => 'list_products_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sneaker' ),
	'description' => __( 'Show multiple products by ID or SKU.', 'sneaker' ),
	'params'      => array_merge(
		array(
			array(
				'type'        => 'textfield',
			...
			array(
				...
				'save_always' => true,
				'description' => __( 'Enter List of Products', 'sneaker' ),
			),
			array(
				'type'       => 'hidden',
				'param_name' => 'skus',
	...
	vc_map( array(
	'name'        => __( 'RT Product categories', 'sneaker' ),
	'base'        => 'product_categories_r',
	"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
	'category'    => __( 'Theme', 'sneaker' ),
	'description' => __( 'Display product categories loop', 'sneaker' ),
	'params'      => array_merge( 
			array(
				array(
					'type'        => 'textfield',
					'heading'     => __( 'Number', 'sneaker' ),
					'param_name'  => 'number',
					'description' => __( 'The `number` field is used to display the number of products.', 'sneaker' ),
				),
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Order by', 'sneaker' ),
				...
				array(
					...
					'description' => __( 'Hide empty', 'sneaker' ),
				),
				array(
					'type'       => 'autocomplete',
					'heading'    => __( 'Categories', 'sneaker' ),
					'param_name' => 'ids',
					'settings'   => array(
						'multiple' => true,
						'sortable' => true,
					...
					'save_always' => true,
					'description' => __( 'List of product categories', 'sneaker' ),
				), 
			),road_get_slider_setting()
		)
	)  );
		...
		vc_map( array(
		'name'        => __( 'RT Product category', 'sneaker' ),
		'base'        => 'product_category_r',
		"icon"        => get_template_directory_uri() . "/images/road-icon.jpg",
		'category'    => __( 'Theme', 'sneaker' ),
		'description' => __( 'Show multiple products in a category', 'sneaker' ),
		'params'      => array_merge(
			array(
				array(
					'type'        => 'textfield',
				...
				array(
					'type'        => 'dropdown',
					'heading'     => __( 'Category', 'sneaker' ),
					'value'       => $product_categories_dropdown,
					'param_name'  => 'category',
					'save_always' => true,
					'description' => __( 'Product category list', 'sneaker' ),
				),
			),road_get_slider_setting()
		)
	)  );
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $product_object ) ) {
				...
				if ( ! empty( $product_sku ) ) {
					$product_sku_display = ' - ' . __( 'Sku', 'sneaker' ) . ': ' . $product_sku;
				}

				$product_title_display = '';
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . __( 'Title', 'sneaker' ) . ': ' . $product_title;
				}

				$product_id_display = __( 'Id', 'sneaker' ) . ': ' . $product_id;
	...
	public function productCategoryCategoryAutocompleteSuggester( $query, $slug = false ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = __( 'Id', 'sneaker' ) . ': ' . $value['id'] . ( ( strlen( $value['name'] ) > 0 ) ? ' - ' . __( 'Name', 'sneaker' ) . ': ' . $value['name'] : '' ) . ( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . __( 'Slug', 'sneaker' ) . ': ' . $value['slug'] : '' );
				$result[] = $data;
			}
		}
	...
	protected function productCategoryTermOutput( $term ) {
		...
		$term_slug_display = '';
		if ( ! empty( $term_slug ) ) {
			$term_slug_display = ' - ' . __( 'Sku', 'sneaker' ) . ': ' . $term_slug;
		}

		$term_title_display = '';
		if ( ! empty( $term_title ) ) {
			$term_title_display = ' - ' . __( 'Title', 'sneaker' ) . ': ' . $term_title;
		}

		$term_id_display = __( 'Id', 'sneaker' ) . ': ' . $term_id;
	...
	public static function getProductsFieldsList() {
		return array(
			__( 'SKU', 'sneaker' )               => 'sku',
			__( 'ID', 'sneaker' )                => 'id',
			__( 'Price', 'sneaker' )             => 'price',
			__( 'Regular Price', 'sneaker' )     => 'regular_price',
			__( 'Sale Price', 'sneaker' )        => 'sale_price',
			__( 'Price html', 'sneaker' )        => 'price_html',
			__( 'Reviews count', 'sneaker' )     => 'reviews_count',
			__( 'Short description', 'sneaker' ) => 'short_description',
			__( 'Dimensions', 'sneaker' )        => 'dimensions',
			__( 'Rating count', 'sneaker' )      => 'rating_count',
			__( 'Weight', 'sneaker' )            => 'weight',
			__( 'Is on sale', 'sneaker' )        => 'on_sale',
			__( 'Custom field', 'sneaker' )      => '_custom_',
		);
	}
	...
	public static function getOrderFieldsList() {
		return array(
			__( 'ID', 'sneaker' )                       => 'id',
			__( 'Order number', 'sneaker' )             => 'order_number',
			__( 'Currency', 'sneaker' )                 => 'order_currency',
			__( 'Total', 'sneaker' )                    => 'total',
			__( 'Status', 'sneaker' )                   => 'status',
			__( 'Payment method', 'sneaker' )           => 'payment_method',
			__( 'Billing address city', 'sneaker' )     => 'billing_address_city',
			__( 'Billing address country', 'sneaker' )  => 'billing_address_country',
			__( 'Shipping address city', 'sneaker' )    => 'shipping_address_city',
			__( 'Shipping address country', 'sneaker' ) => 'shipping_address_country',
			__( 'Customer Note', 'sneaker' )            => 'customer_note',
			__( 'Customer API', 'sneaker' )             => 'customer_api',
			__( 'Custom field', 'sneaker' )             => '_custom_',
		);
	}
